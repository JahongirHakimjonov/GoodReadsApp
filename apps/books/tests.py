...
class BookListViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.book1 = Book.objects.create(title='Test Book 1', slug='test-book-1')
        self.book2 = Book.objects.create(title='Test Book 2', slug='test-book-2')
...
class BookDetailViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.book = Book.objects.create(title='Test Book', slug='test-book')

    def test_book_detail_view(self):
        response = self.client.get(reverse('books:book-detail', kwargs={'slug': 'test-book'}))
        self.assertEqual(response.status_code, 200)
